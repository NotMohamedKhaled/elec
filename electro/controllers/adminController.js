...
const createProduct = async (req, res) => {
  ...
  try {
    const { title, category, price, description, specs } = req.body;

    // Use uploaded file or fall back to img URL in body
    let imgUrl = req.body.img || '';
    if (req.file) {
      imgUrl = `http://localhost:5000/uploads/${req.file.filename}`;
    }

    if (!imgUrl) return res.status(400).json({ error: 'Product image is required' });

    // Parse specs — can be comma-separated string or JSON array
    let specsList = [];
    if (specs) {
      try { specsList = JSON.parse(specs); } catch { specsList = specs.split(',').map(s => s.trim()).filter(Boolean); }
    }
